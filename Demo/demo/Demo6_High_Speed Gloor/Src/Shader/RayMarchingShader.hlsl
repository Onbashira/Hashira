...
float map(float3 p)
{
    ...
    float d1 = 0; // ifsBox(trans(p, 8.));
    float d2 = sdBox(p, 0.3);
    return max(d1, d2);
...
PSOut PS_Main(VSOut input)
{
    ...
    //Screen pos
    float2 fixedPos = Resolution - input.position.xy * 0.5 ;
    float2 p = (fixedPos.xy * 2.0 - Resolution) / min(Resolution.x, Resolution.y);
    //float2 p = input.texcoord;

    float fovValue = cos(sin(Time * 2.0) * saw);

	//Camera
    float3 cameraPos = float3(0,0, 3.0);
    ...
    {
        dist = map(rotate(rayPos, radians(Time * 10), float3(0.0, 0.0, 1.0)));
    ...
    color = float3(ac * 0.01, ac * 0.02, ac * 0.01 );
